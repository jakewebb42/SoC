...
PwmCore::PwmCore(uint32_t core_base_addr) {
   base_addr = core_base_addr;
   set_freq(50); // what the servos use as a base
   
   ch_servo0 = 0; // Front Left
   ch_servo1 = 1; // Front Right
   ch_servo2 = 2; // Back Left
   ch_servo3 = 3; // Back Right
}
...
void PwmCore::set_duty_wheels(double f) {
    int duty;
    duty = (int) (f * MAX);
    
    set_duty(duty, ch_servo0);
    set_duty(duty, ch_servo1);
    set_duty(duty, ch_servo2);
    set_duty(duty, ch_servo3);    
}